...
class Level(object):
    ...
    def __init__(self, width, height):
        ...
        self.blocks = QtGui.QImage(width, height, QtGui.QImage.Format_RGB32)
    ...
    def putBox(self, coords, size, color = QtGui.qRgb(0,0,0)):
        for i, j in itertools.product(range(coords[0], coords[0]+size), range(coords[1], coords[1]+size)):
            self.blocks.setPixel(i, j, color)

    def putWall(self, coords, length, blocks = [(1, (1,1), QtGui.qRgb(0,0,0))]):
        coords = list(coords)
        for i, (b, s, c) in zip(range(length), itertools.cycle(blocks)):
            if b:
                self.putBox(coords, b, c)
                self.putBox(self.mirror(coords, b), b, c)
            coords[0] += s[0]
            coords[1] += s[1]

    def generate(self):
        self.blocks.fill(QtGui.qRgb(255,255,255))
        ...
        while True:
            steps = [
                ...
                (1, (1,0), QtGui.qRgb(128,128,128)),
                (1, (0,1), QtGui.qRgb(128,128,128)),

                (1, (1,0), QtGui.qRgb(0,0,0)),
                (1, (0,1), QtGui.qRgb(0,0,0)),
           
                (2, (2,2), QtGui.qRgb(0,0,0))
            ]
            templates = list(itertools.product(steps, repeat=2))
            for i in range(random.randrange(10, 20)):
                self.putWall(self.randcoords(), random.randrange(4,12), random.choice(templates)) 
