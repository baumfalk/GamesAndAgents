...
def attack_rule_1(bot,commander,knowledgeBase):
    ...
    if((commander.game.team.flag.carrier != None) and knowledgeBase.atMidsection(bot)):
        commander.issue(orders.Attack, bot, commander.game.enemyTeam.flagScoreLocation ,description = "Attacker" + bot.name + "charge to enemy scoring point")
...
def attack_rule_5(bot,commander,knowledgeBase):
    ...
    if((commander.game.team.flag.carrier != None)):
        ...
        commander.issue(orders.Attack, bot, loc,description = "Attacker" + bot.name + "charge to enemy carrier")
       # knowledgeBase.updateStatistics(things)
        return True
    return False
...
def attack_rule_6(bot,commander,knowledgeBase):
    ...
    if( (commander.game.enemyTeam.flag.carrier == None)):
        ...
        commander.issue(orders.Attack, bot, loc,description = "Attacker" + bot.name + "attcking towards to enemy flag")
       # knowledgeBase.updateStatistics(things)
        return True
    return False
